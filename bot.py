...
async def db_init(dsn: str):
    ...
    async with _pool.acquire() as conn:
        ...
        # каналы, которые админ явно удалил через /delchannel — чтобы они не
        # "воскресали" сами по себе (например, из-за старого CHANNEL_ID в .env
        # при каждом рестарте, или из-за повторного события смены прав бота)
        await conn.execute(
            """
...
async def add_channel(channel_id: int, title: Optional[str]) -> None:
    # уважаем явное удаление админом — не даём каналу "воскреснуть" само собой
    if await is_channel_removed(channel_id):
        return
    async with _pool.acquire() as conn:
        await conn.execute(
            """
...
async def remove_channel(channel_id: int, permanent: bool = False) -> None:
    async with _pool.acquire() as conn:
        await conn.execute("DELETE FROM channels WHERE channel_id = $1", channel_id)
        if permanent:
            await conn.execute(
                "INSERT INTO removed_channels (channel_id) VALUES ($1) ON CONFLICT DO NOTHING",
                channel_id,
            )


async def is_channel_removed(channel_id: int) -> bool:
    async with _pool.acquire() as conn:
        row = await conn.fetchrow("SELECT 1 FROM removed_channels WHERE channel_id = $1", channel_id)
        return row is not None


async def restore_channel(channel_id: int) -> None:
    """Снять пометку 'удалён навсегда' — используется, когда бота заново делают админом канала."""
    async with _pool.acquire() as conn:
        await conn.execute("DELETE FROM removed_channels WHERE channel_id = $1", channel_id)
...
class TouchUserMiddleware(BaseMiddleware):
    async def __call__(self, handler, event: Message, data: dict):
        # Считаем "писал боту" только тех, кто реально открыл с ним личку —
        # иначе сюда попадают все, чьи сообщения в служебной группе/канале
        # бот просто видит, а /broadcast потом не может им написать первым
        # (Telegram: "bot can't initiate conversation with a user").
        if event.from_user and not event.from_user.is_bot and event.chat.type == ChatType.PRIVATE:
            try:
...
async def on_membership_changed(update: ChatMemberUpdated):
    ...
    if new_status == ChatMemberStatus.ADMINISTRATOR:
        if old_status in (ChatMemberStatus.LEFT, ChatMemberStatus.KICKED, ChatMemberStatus.MEMBER):
            # бота реально заново пригласили админом — это осознанное действие,
            # снимаем прежнюю пометку "удалён навсегда", если она была
            await restore_channel(update.chat.id)
...
async def cmd_delchannel(message: Message):
    ...
    await remove_channel(channel_id, permanent=True)
    await message.answer(
        f"✅ Канал {channel_id} убран из списка публикации навсегда.\n"
        "Он не появится снова сам по себе — ни при рестарте бота, ни при повторных "
        "событиях смены его прав в этом канале. Если захотите вернуть канал — "
        "снимите с бота права админа там и назначьте админом заново."
    )
...
async def finalize_submission(bot: Bot, chat_id: int, from_user, state: FSMContext):
    ...
    author_display = f"@{from_user.username}" if from_user.username else from_user.full_name
    ...
    info_text = (
        ...
        f"Файлов: {len(media)}\n\n"
        "⬆️ Выше — обложка с названием/хэштегом/автором (как будет выглядеть пост), "
        "ниже — сам контент, который предлагают."
    )
    for admin_id in await get_admin_ids():
        try:
            # сначала — как будет выглядеть готовый пост
            await bot.send_photo(admin_id, cover_file_id, caption=post_preview_caption, parse_mode=ParseMode.HTML)
            # затем — сам контент, который прислал автор заявки
            for item in media:
            ...
            continue  # этот конкретный админ ещё не писал боту /start — идём к следующему
...
async def _broadcast_send(bot: Bot, uid: int, source: Optional[Message], text: Optional[str]) -> bool:
    for _ in range(2):  # одна попытка + один retry, если Telegram попросил подождать
        try:
            ...
            # Любая другая ошибка (сеть, таймаут и т.п.) не должна ронять
            # весь asyncio.gather() и обрывать рассылку остальным.
            log.warning("Broadcast: не смог отправить id%s: %s", uid, e)
            return False
    return False
...
async def main():
    ...
    if LEGACY_CHANNEL_ID and not await is_channel_removed(LEGACY_CHANNEL_ID):
